...
#include "sitaw.h"
#include "commander.h"
// following lines are implemented to enable the Countdown and other experimental tasks
#include "FreeRTOS.h"
#include "task.h"
...
LOG_ADD(LOG_UINT8, ARDetected, &sitAwARAccZ.released)
LOG_ADD(LOG_UINT8, TuDetected, &sitAwTuAngle.released)
LOG_ADD(LOG_FLOAT, High, &High)
LOG_ADD(LOG_FLOAT, StartAltitude, &start_altitude)
LOG_ADD(LOG_FLOAT, Difference, &difference)
...
#if defined(SITAW_ENABLED)
//static state_s ss; // --> ss.start_altitude
float GetEstimatedPositionAsFunction(); //created in and imported from position_estimator_altitude.c go to get altitude-level
#endif
...
difference = High - start_altitude;
#if defined(SITAW_ENABLED)
	...
	if (sitAwARDetected()) {
     ...
     if(sitAwTuDetected()) {
      ...
      if ((actualTick > ((unsigned int)((4000)*(( ( TickType_t ) 1000 )/1000.0))))&& ((actualTick - countdown.timestamp) < ((unsigned int)((4000)*(( ( TickType_t ) 1000 )/1000.0)))) &&!(sitAwTuDetected())) {

    	  setpoint->mode.z = modeVelocity;  //while timer is on and CF is not near ground, the althold mode is activated
    	  setpoint->velocity.z = -0.2;	   //Velocity of the z-axis is set to 0 first, but..
      if ((1 < (difference)) && ((difference) < 1.5)) {
    	  setpoint->velocity.z = -0.2;     //.. then depends on level above groundlevel..
      }
      if ((1.5 < (difference)) && ((difference) < 3.5)) {
         	  setpoint->velocity.z = -0.3; // .. and the higher the level..
           }
      if ((3.5 < (difference)) && ((difference) < 5.5)) {
         	  setpoint->velocity.z = -0.4; // .. the higher the downward-speed.
           }
      if ((5.5 < (difference)) && ((difference) < 7.5)) {
