...
static void sitAwPreThrustUpdateCallOut(setpoint_t *setpoint)
{
  /* Code that shall run BEFORE each thrust distribution update, should be placed here. */
	...
	if ((actualTick == 1)){
		...
		start_altitude = GetEstimatedPositionAsFunction();
	}
     if(sitAwTuDetected()) {
      ...
      if (((actualTick - countdown.timestamp) < ((unsigned int)((2000)*(( ( TickType_t ) 1000 )/1000.0))))) {
    	  High = GetEstimatedPositionAsFunction();
      ...
      if ((0.2 < (start_altitude - High)) && ((start_altitude- High) < 1)) {
    	  setpoint->velocity.z = -0.2;     //.. then depends on level above groundlevel..
      }
      if ((1 < (start_altitude - High)) && ((start_altitude - High) < 2)) {
         	  setpoint->velocity.z = -0.3; // .. and the higher the level..
           }
      if ((2 < (start_altitude - High)) && ((start_altitude - High) < 3)) {
         	  setpoint->velocity.z = -0.4; // .. the higher the downward-speed.
           }
      if ((3 < (start_altitude - High)) && ((start_altitude - High) < 4)) {
         	  setpoint->velocity.z = -0.5;
           }
      if (4 < (start_altitude - High)) {
         	  setpoint->velocity.z = -0.6;
           }
      }
      if ((start_altitude - High) < 0.2) {
